...
| 재정렬(rerank) | 검색 결과를 더 정확한 순서로 다시 매기는 별도 단계. Cohere `rerank-v4.0` 사용. **데이터 모델(표 구조)과는 무관**하므로 여기선 다루지 않음. |
...
**무엇을 담나**: 어떤 사이트에서, 어떤 규칙으로 공고를 긁어올지 정의한 목록입니다.
...
> 참고: 검색 후 더 정확한 순서로 다시 매기는 **재정렬(rerank)** 은 Cohere `rerank-v4.0`이 담당하는 별도 단계이며, 이 함수나 표 구조와는 무관합니다.
