...
> 기준 시점: 2026-06-30 (커밋 `d32322c` — 법령 참조를 답변 인용·검증 단일 소스로 통합).
...
| 분기 | 조건 | 동작 |
|---|---|---|
| 📘 **규정** | 규정 관련도 ≥ 0.33 | 내부 규정으로 답변·참조 |
| ⚖️ **법령** | 규정 관련도 < 0.33 **그리고** 서비스 범위 내 | 법제처 법령·판례로 답변, 인용 검증으로 참조 |
| 🚫 **범위 밖** | 규정 관련도 < 0.33 **그리고** 범위 밖(잡담 등) | 근거 미주입 → 정중한 거절, 참조 없음 |

> 설계 메모: 과거에는 "LLM이 규정만으로 충분한지" 판정하는 sufficiency 게이트가 있었으나,
> 재정렬 청크 변동에 민감해 같은 질의가 토큰 하나로 규정↔법령을 오갔다. 현재는 **안정적인
> 점수 신호(maxScore vs 0.33)** 단일 기준만 쓴다. (커밋 `e7a5ef2`)
...
| `RELEVANCE_THRESHOLD` | 0.33 | 규정↔법령 분기 + 법령 노출 게이트 + 판례 필터의 공통 문턱 |
