/**
 * 우리 내부 searchAiLaw 결과를 법제처 공식 원문(lawService.do=get_law_text,
 * korean-law-mcp 의 2번째 콜과 동일)과 대조해 조문 본문 정확성을 검증한다.
 *
 * 검증: 우리 aiSearch 조문 발췌가 공식 원문과 글자 단위로 일치하는지(클립 제외).
...
import { searchAiLaw, type LawRef } from "@/lib/law/search";
...
async function verifyQuery(query: string) {
  console.log(`\n${"═".repeat(72)}\n질의: "${query}"\n${"═".repeat(72)}`);
  const lookup = await searchAiLaw(query);
  if (lookup.refs.length === 0) {
    console.log("  (법령 결과 없음)");
    return;
  }

  for (let i = 0; i < lookup.refs.length; i++) {
    const ref: LawRef = lookup.refs[i];
    console.log(
      `\n■ ${ref.name} (관련도 ${Math.round((ref.score ?? 0) * 100)}%, 법령ID ${ref.lawId})`,
    );
    const blocks = [...(lookup.articles[i] ?? "").matchAll(
      /\[([^\]]+)\]\n([\s\S]*?)(?=\n\[|$)/g,
    )];
    for (const m of blocks) {
      const label = m[1];
      const ourBody = m[2];
      const am = label.match(/제(\d+)조(?:의(\d+))?/);
      if (!am) continue;
      const no = parseInt(am[1], 10);
      const branch = am[2] ? parseInt(am[2], 10) : 0;
      const official = await officialArticle(ref.lawId, no, branch);
      if (!official) {
        console.log(`   제${no}조${branch ? `의${branch}` : ""} · 공식원문 회수 실패 ⚠`);
        continue;
      }
      // 한글·숫자만 남겨 구두점/메타데이터 차이를 제거하고 본문 식별 구절로 대조.
      const k = (s: string) => s.replace(/[^가-힣0-9]/g, "");
      const ourBodyNoTitle = ourBody.replace(/^\s*제\d+조(?:의\d+)?\s*\([^)]*\)/, "");
      ...
      console.log(
        `   ${label}\n     → 원문 대조: ${match ? "✓ 일치(본문이 공식 원문에 그대로 존재)" : "✗ 불일치"} ` +
          `(우리 발췌 ${ourBody.length}자, 원문 ${official.length}자)`,
      );
      if (!match) {
